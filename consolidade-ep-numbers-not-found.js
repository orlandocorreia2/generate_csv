...
const init = async () => {
  ...
  deleteFile("EP-NUMEROS-NAO-ENCONTRADOS-CONSOLIDADO.csv");

  await getDataExtraction({
    filePath: "files/email/EP-NÚMEROS-NÃO-ENCONTRADOS.csv",
    fn: (data) => {
      numbersNotFound[`18${data.valor}`] = true;
    },
  });

  await getDataExtraction({
    filePath: "files/jupyter_colunado/resultado-42088.csv",
    fn: (data) => {
      const ctr = data.ctr;
  ...
  await getDataExtraction({
    filePath: "files/jupyter_colunado/dimensao-42088.csv",
    fn: (data) => {
  ...
  if (dataGenerateFile.length) {
    generateCSV({
      filePath: "EP-NUMEROS-NAO-ENCONTRADOS-CONSOLIDADO",
